(() => {
    ...
    const slidersArray = document.querySelectorAll(".hidden-slide");
    const windowInnerWidth = document.documentElement.clientWidth;
    const photosArrayAll = document.querySelectorAll(".hidden-photo");
    const photosArray = [];
    photosArrayAll.forEach((item => {
        photosArray.push(item);
    }));
    if (windowInnerWidth <= 768) slidersArray.forEach((item => {
        item.classList.remove("_hidden");
    })); else if (windowInnerWidth > 768) slidersArray.forEach((item => {
        item.classList.add("_hidden");
    }));
    const fullSlidersArray = document.querySelectorAll(".swiper-slide");
    const fullSlidersLiveArray = [];
    fullSlidersArray.forEach((slide => {
        fullSlidersLiveArray.push(slide);
    }));
    let selectItem;
    let indexSelectItem;
    photosArray.forEach((item => {
        item.addEventListener("click", (function(event) {
            selectItem = item;
            for (let i = 0; i <= photosArray.length; i++) if (photosArray[i] === event.target) {
                indexSelectItem = i;
                for (let j = 0; j < fullSlidersLiveArray.length; j++) fullSlidersLiveArray[j].classList.add("_hidden");
                fullSlidersLiveArray[indexSelectItem].classList.remove("_hidden");
            }
        }));
    }));
